...
public:
    ...
    bool warning_checkbox(std::string_view label, bool* callback);

    bool warning_checkbox(std::string_view label, bool* callback, int* key, int* mode);
    ...
    bool checkbox(std::string_view label, bool* callback, std::vector<float*> colors, bool active_alpha = true);
...
public:
    ...
    void shadow_line(ImDrawList* draw_list, const ImVec2& p1, const ImVec2& p2, ImU32 shadow_col, float shadow_thickness, const ImVec2& shadow_offset = ImVec2(0, 0));
    ...
    void ImU32ToFloat(ImU32 in, float out[4]);

    ImU32 FloatToImU32(float in[4]);
...
{
public:
    void add(const std::string& title, const std::string& message, float time = 5.0f, notify_pos_t pos = notify_pos_t::top_right);
    void render();
private:
