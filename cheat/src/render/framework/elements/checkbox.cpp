...
bool c_gui::checkbox(std::string_view label, bool* callback)
{
    ...
    draw->text_clipped_outline(window->DrawList, var->font.tahoma, ImVec2(clickable.Max.x + elements->widgets.padding.x, rect.Min.y), rect.Max, GetColorU32(ImGuiCol_Text), label.data(), NULL, NULL, ImVec2(0.f, 0.5f));

    return pressed;
}

bool c_gui::warning_checkbox(std::string_view label, bool* callback)
{
    gui->push_style_color(ImGuiCol_Text, draw->get_clr(clr->widgets.warning));
    bool result = gui->checkbox(label, callback);
    gui->pop_style_color();
    return result;
}

bool c_gui::warning_checkbox(std::string_view label, bool* callback, int* key, int* mode)
{
    gui->push_style_color(ImGuiCol_Text, draw->get_clr(clr->widgets.warning));
    bool result = gui->checkbox(label, callback, key, mode);
    gui->pop_style_color();
    return result;
}
...
bool c_gui::checkbox(std::string_view label, bool* callback, std::vector<float*> colors, bool active_alpha)
{
    float y_pos = GetCursorPosY();
    gui->checkbox(label, callback);
    ...
    {
        float offset = (colors.size() - i) * (elements->widgets.color_size.x + elements->widgets.spacing.x);
        set_cursor_pos(ImVec2(GetWindowWidth() - offset - GetStyle().WindowPadding.x + elements->widgets.spacing.x, y_pos));
        gui->color_edit((std::stringstream{} << label << "color" << i).str().c_str(), colors[i], active_alpha);
    }
