...
void c_draw::text_outline(ImDrawList* draw_list, const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect)
{
    ...
    {
        float thickness = var->window.text_outline_thickness;
        ImU32 outline_col = get_clr(clr->widgets.text_outline.Value);

        for (float x = -thickness; x <= thickness; x += thickness) {
            for (float y = -thickness; y <= thickness; y += thickness) {
                if (x == 0 && y == 0) continue;
                text(draw_list, font, font_size, ImVec2(std::round(pos.x + x), std::round(pos.y + y)), outline_col, text_begin, text_end, wrap_width, cpu_fine_clip_rect);
            }
        }
    }
    text(draw_list, font, font_size, ImVec2(std::round(pos.x), std::round(pos.y)), col, text_begin, text_end, wrap_width, cpu_fine_clip_rect);
}
...
void c_draw::text_outline(const char* text)
{
    if (var->window.text_outline)
    {
        ...
        for (float x = -thickness; x <= thickness; x += thickness) {
            for (float y = -thickness; y <= thickness; y += thickness) {
                if (x == 0 && y == 0) continue;
                GetWindowDrawList()->AddText(NULL, 0, ImVec2(std::round(pos.x + x), std::round(pos.y + y)), outline_col, text);
            }
        }
    }
    Text(text);
...
void c_draw::text_clipped_outline(ImDrawList* draw_list, ImFont* font, const ImVec2& pos_min, const ImVec2& pos_max, ImU32 color, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    if (var->window.text_outline)
    {
        ...
        for (float x = -thickness; x <= thickness; x += thickness) {
            for (float y = -thickness; y <= thickness; y += thickness) {
                if (x == 0 && y == 0) continue;
                text_clipped(draw_list, font, pos_min + ImVec2(x, y), pos_max + ImVec2(x, y), outline_col, text, text_display_end, text_size_if_known, align, clip_rect);
            }
        }
    }
    text_clipped(draw_list, font, pos_min, pos_max, color, text, text_display_end, text_size_if_known, align, clip_rect);
}
...
void c_draw::shadow_line(ImDrawList* draw_list, const ImVec2& p1, const ImVec2& p2, ImU32 shadow_col, float shadow_thickness, const ImVec2& shadow_offset)
{
    ImVec2 points[2] = { p1, p2 };
    shadow_convex_poly(draw_list, points, 2, shadow_col, shadow_thickness, shadow_offset);
}
...
void c_draw::ImU32ToFloat(ImU32 in, float out[4])
{
    ImVec4 col = ImGui::ColorConvertU32ToFloat4(in);
    out[0] = col.x;
    out[1] = col.y;
    out[2] = col.z;
    out[3] = col.w;
...
ImU32 c_draw::FloatToImU32(float in[4])
{
...
void c_draw::window_decorations()
{
    ...
    {
        ImU32 overlay_top = draw->get_clr(ImVec4(clr->accent.Value.x, clr->accent.Value.y, clr->accent.Value.z, var->window.acrylic_intensity * 0.15f));
        ImU32 overlay_bot = draw->get_clr(ImVec4(clr->accent.Value.x, clr->accent.Value.y, clr->accent.Value.z, var->window.acrylic_intensity * 0.05f));

        draw->fade_rect_filled(GetWindowDrawList(), GetWindowPos() + ImVec2(6, var->window.titlebar), GetWindowPos() + GetWindowSize() - ImVec2(6, 6), overlay_top, overlay_bot, fade_direction::vertically);
    }
    ...
    if (ItemAdd(cr, cid, NULL, ImGuiItemFlags_NoNav))
    {
        ...
        if (pressed)
        {
            if (strcmp(w->Name, "Hone") == 0) var->gui.current_section[0] = false;
            else if (strcmp(w->Name, "other") == 0) var->gui.current_section[1] = false;
            else if (strcmp(w->Name, "Preview") == 0) var->gui.current_section[2] = false;
            else if (strcmp(w->Name, "Explore") == 0) var->gui.current_section[4] = false;
            else if (strcmp(w->Name, "Style") == 0) var->gui.current_section[5] = false;
            else if (strcmp(w->Name, "cloud") == 0) var->gui.current_section[6] = false;
            else if (strcmp(w->Name, "Game") == 0) var->gui.menu_opened = false;
        }
    }
}
