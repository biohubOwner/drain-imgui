...
void c_gui::begin_content()
{
    ...
    float content_w = GetWindowWidth() - elements->content.window_padding.x * 2;
    if (var->window.clamp_content)
        content_w = ImMin(content_w, (float)var->window.max_width);
    gui->begin_def_child("content area", ImVec2(content_w, GetContentRegionAvail().y - elements->content.window_padding.y * 2), ImGuiChildFlags_None, ImGuiWindowFlags_NoNav | ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoMove);
...
bool c_gui::selectable_ex(const char* label, bool active, const ImVec2& size)
{
    ...
    const float width = size.x > 0 ? size.x : GetContentRegionAvail().x;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, pos + ImVec2(width, size.y));
    ...
    draw->text_clipped_outline(window->DrawList, var->font.tahoma, rect.Min + ImVec2(10, -1), rect.Max - ImVec2(10, 1), draw->get_clr(active ? clr->accent : hovered ? clr->widgets.text_inactive : clr->widgets.text), label, NULL, NULL, ImVec2(0.0f, 0.5f));
