...
bool c_gui::keybind(const char* label, int* key, int* mode)
{
    ...
    if (*key >= 0 && *key < IM_ARRAYSIZE(keys))
        active_key += keys[*key];
    else
        active_key += "unk";
...
{
    draw->text_outline(GetWindowDrawList(), var->font.tahoma, var->font.tahoma->FontSize, GetCursorScreenPos() + ImVec2(1, 0), GetColorU32(ImGuiCol_Text), label);
